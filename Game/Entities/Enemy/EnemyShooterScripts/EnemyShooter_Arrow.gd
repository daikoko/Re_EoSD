...
@export_group("Arrow")
@export var arrow_size:int = 1
@export var arrow_length:float = 100
@export var arrow_width:float = 100
@export var arrow_displacement:float= 250
@export var arrow_fill:bool = false
...
@export var arrow_override:Dictionary = {
	"Easy":    -1,
	"Normal":  -1,
	"Hard":    -1,
	"Lunatic": -1
...
func set_shooter(enemy:Node2D) -> void:
	Main = GlobalShooter.create_arrow_shooter(
...
func copy() -> Resource:
	...
	if fire_override[difficulty_key] != -1:
		...
		copy.fire_count = snappedi(fire_count * GlobalStage.get_shooter_modifier("fire"), fire_factor)
	...
	if arrow_override[difficulty_key] != -1:
		copy.arrow_size = arrow_override[difficulty_key]
	else:
		copy.arrow_size = round(
			arrow_size * GlobalStage.get_shooter_modifier("arrow")
		)
