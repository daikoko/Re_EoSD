...
const MARISA_MAIN     := preload("res://Game/Entities/Player/PlayerResources/Marisa/ShotA/Shot/Bullet_MarisaMain.tres")
const MARISA_MISSLE   := preload("res://Game/Entities/Player/PlayerResources/Marisa/ShotA/Shot/Bullet_MarisaMissle.tres")

const MARISA_SIDE     := preload("res://Game/Entities/Player/PlayerResources/Marisa/ShotB/Shot/Side_MarisaLaser.tscn")
...
func _ready():
	...
	var side_01 = MARISA_SIDE.instantiate()
	side_01.laser_hit.connect(_on_MarisaSide_laser_hit)
	var side_02 = MARISA_SIDE.instantiate()
	side_02.laser_hit.connect(_on_MarisaSide_laser_hit)
	var side_03 = MARISA_SIDE.instantiate()
	side_03.laser_hit.connect(_on_MarisaSide_laser_hit)
	var side_04 = MARISA_SIDE.instantiate()
	side_04.laser_hit.connect(_on_MarisaSide_laser_hit)
	...
	%SideShot.set_levels(
		[side_01, side_02, side_03, side_04],
		{
			1: {
				side_01: {
					"transform": Transform2D(deg_to_rad(0), Vector2.ZERO),
					"activated": false
				},
				side_02: {
					"transform": Transform2D(deg_to_rad(0), Vector2.ZERO),
...
func _on_MarisaSide_laser_hit(collider,identity):
	if identity == "Enemy":
		if focusing:
			collider.hit(SIDE_FOCUS_DAMAGE)
		else:
			collider.hit(SIDE_BASE_DAMAGE + (SIDE_BASE_EXTRA * power))
