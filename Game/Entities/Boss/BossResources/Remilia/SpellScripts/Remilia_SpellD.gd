...
const HELPER_05 := preload("res://Game/Entities/Boss/BossResources/Remilia/SpellResources/RemiliaHelper05.tscn")
...
@export var A_primary_layout_line_count:int
@export var A_secondary_fire_count:int
const A_PRIMARY_BULLET_SPEED := 800
const A_SECONDARY_BULLET_SPEED := 200
const A_SECONDARY_BULLET_SPEED_RANGE := 80
const A_SECONDARY_SHOOTER_ROTATION_RANGE := 5.0
const A_ATTACK_DELAY := 0.8
...
func prepare(EventHandler:Control, BossDict:Dictionary) -> float:
	...
	A_Shooter.RNG = RNG
	A_Shooter.build(
		A_primary_layout_line_count,
		A_SECONDARY_SHOOTER_ROTATION_RANGE
	)
...
func attack_a_loop():
	A_Shooter.fire(
		A_PRIMARY_BULLET_SPEED,
		A_secondary_fire_count,
		A_SECONDARY_BULLET_SPEED,
		A_SECONDARY_BULLET_SPEED_RANGE
	)
	await Boss.create_waiter(A_ATTACK_DELAY).finished
