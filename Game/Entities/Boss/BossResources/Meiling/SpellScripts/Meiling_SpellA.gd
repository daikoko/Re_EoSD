...
@export var A_fire_time:float = 0.1
const A_LAYOUT_COLUMN_COUNT := 2
const A_LAYOUT_COLUMN_RANGE := 60
const A_BULLET_SPEED := 200
const A_BULLET_SPEED_RANGE := 20
const A_SHOOTER_ROTATION_SPEED  := 120.0
...
const B_BULLET_GRAVITY := 0.6

const PREPARE_WAIT      := 0.2
...
func prepare(EventHandler:Control, BossDict:Dictionary) -> float:
	...
	A_Shooter.rotation_speed = A_SHOOTER_ROTATION_SPEED
	A_Shooter.RNG = RNG
