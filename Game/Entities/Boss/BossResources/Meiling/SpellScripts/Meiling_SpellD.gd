...
func prepare(EventHandler:Control, BossDict:Dictionary) -> float:
	...
	var interval = floori(A_layout_spawner_count / 6)
	var column_data:Array[BulletData] = []
	for i in A_layout_spawner_count:
	...
	B_Shooter = GlobalShooter.create_linear_shooter(1)
	var b_bullets_base = [
		GlobalShooter.BRIGHT_WHITE,
		GlobalShooter.BRIGHT_WHITE,
		GlobalShooter.BRIGHT_WHITE,
		GlobalShooter.BRIGHT_WHITE,
		GlobalShooter.BRIGHT_WHITE,
...
func attack_a():
	...
	var angle_to_player = GlobalPlayer.angle_to_player(Boss.global_position)
	A_Shooter.rotation = (
		angle_to_player + 
		(deg_to_rad(A_SHOOTER_ROTATION_START) * A_direction)
	)
	A_Shooter.rotation_speed = deg_to_rad(
		(A_SHOOTER_ROTATION_ARC / A_FIRE_DURATION) * A_direction * -1
	)
...
func attack_b() -> void:
	for i in B_fire_count:
		...
		await Boss.create_waiter(B_FIRE_DURATION / B_fire_count).finished
