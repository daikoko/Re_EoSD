...
const HELPER_05 := preload("res://Game/Entities/Boss/BossResources/Cirno/SpellResources/CirnoHelper05.tscn")
...
const DISTANCE     := 180
const TIME         := 0.6
const RAND_SEED    := 71162

var A_Shooter:Node2D
@export_group("Shooter_A")
@export var A_arm_count:int = 9
@export var A_body_count:int = 36
const A_EDGE_COUNT := 6
const A_BODY_SPEED := 160.0
const A_EDGE_SPEED := 220.0
...
func prepare(EventHandler:Control, BossDict:Dictionary) -> float:
	...
	A_Shooter.set_snowflake(A_arm_count, RNG)
...
func attack_a() -> void:
	...
	A_Shooter.spawn_bullets(
		A_body_count, A_BODY_SPEED, A_EDGE_COUNT, A_EDGE_SPEED
	)
	await A_Shooter.finished_spawning
