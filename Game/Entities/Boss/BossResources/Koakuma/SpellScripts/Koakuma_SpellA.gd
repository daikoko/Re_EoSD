...
const SPELL_PORTRAIT := preload("res://Game/Entities/Boss/BossResources/Koakuma/Sprite/SpellPortrait_Koakuma.tres")
...
func attack_a() -> void:
	A_Shooter.rotation = (
		...
		(0.5 * A_direction * deg_to_rad(A_SHOOTER_ROTATION_ARC))
	)
	A_Shooter.rotation_speed = (
		A_direction * deg_to_rad(A_SHOOTER_ROTATION_ARC / A_FIRE_DURATION)
	)
...
func attack_b() -> void:
	...
	B_Shooter.rotation_random_range = Vector2(0, TAU)
