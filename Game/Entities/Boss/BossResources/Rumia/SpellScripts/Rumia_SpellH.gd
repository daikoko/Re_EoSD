...
const HELPER_09 := preload("res://Game/Entities/Boss/BossResources/Rumia/SpellResources/RumiaHelper09.tscn")
...
func attack_a() -> void:
	...
	Boss.return_animation()
	await Boss.create_waiter(AFTER_ATTACK_WAIT).finished
	await Boss.create_waiter(A_SHOOTER_WAIT).finished
