...
const HELPER_17 := preload("res://Game/Entities/Boss/BossResources/Rumia/SpellResources/RumiaHelper17.tscn")
...
func prepare(EventHandler:Control, BossDict:Dictionary) -> float:
	...
	A_Shooter = HELPER_17.instantiate()
	A_Shooter.RNG = RNG
...
func start() -> void:
	...
	spell_started.emit()
	Boss.enable()
	await Boss.move_boss(Vector2(400, 270), 0.4).finished
	
	A_Shooter.start()
	await A_Shooter.start_finished
...
func attack_a():
	...
	A_Shooter.fire()
	await A_Shooter.shooting_finished
