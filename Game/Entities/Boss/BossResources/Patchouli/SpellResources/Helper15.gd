...
const A_LAYOUT_SPAWNER_COUNT := 1
const A_LAYOUT_SHOT_RANGE := 360
const A_FIRE_TIME := 0.1
const A_BULLET_SPEED := 180
const A_BULLET_SPEED_RANGE := 40
const A_SHOOTER_ROTATION := 110
...
func _ready() -> void:
	...
	A_Bullets = RowData_Column.new([ColumnData_Bullet.new([GlobalShooter.SPADE_YELLOW])])
