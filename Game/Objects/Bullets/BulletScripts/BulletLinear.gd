...
func _process(delta):
	...
	velocity = direction.rotated(dir) * speed
	
	rot += rot_speed * delta
	rotation = direction.angle() + dir + rot
...
func activate(
		...
		speed:float, dir:float, rot:float, rot_speed:float,
		delay:float, duration:float, speed_change:float, dir_change:float, 
...
func deactivate() -> void:
	GlobalPool.bullet_linear_despawned.emit(id)
...
func _on_FlashTween_finished():
	...
	SpeedTween.tween_interval(delay)
	SpeedTween.tween_property(self, "speed", speed + speed_change, duration)
	SpeedTween.parallel().tween_property(self, "dir", dir + dir_change, duration)
