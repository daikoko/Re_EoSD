...
func _process(delta):
	...
	var sine_position = Vector2(distance, amplitude * sin(0.01 * compression * distance))
	sine_position = sine_position.rotated(origin_rotation)
	
	position = origin + sine_position
...
func activate(data:BulletData, transform, 
	...
	origin_rotation = transform.get_rotation()
	
	self.speed = speed
	self.distance = 0
	self.amplitude = amplitude
	self.compression = compression
	self.prev_position = origin
...
func deactivate() -> void:
	GlobalPool.bullet_sine_despawned.emit(id)
